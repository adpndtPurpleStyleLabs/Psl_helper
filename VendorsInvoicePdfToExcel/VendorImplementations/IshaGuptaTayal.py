...
class IshaGuptaTayal:
    ...
    def getVendorInfo(self):
        ...
        vendorInfo = firstPageText[indexOfContainsInList(firstPageText, "IGT")][0].split("\n")
        return {
    ...
    def getItemInfo(self):
        ...
        for index in range(1, len(pages) + 1):
            ...
            for itemIndex, item in enumerate(page[indexOfHeader + 1:]):
                if indexOfContainsInList(item, "continue") is not -1 or indexOfContainsInList(item, "total") is not -1:
                    break
                if item[0].strip() == "":
                    continue
    ...
    def getVendorBankInfo(self):
        ...
        bankInfo = lastPage[indexOfContainsInList(lastPage, "bank"):][indexOfContainsInList(lastPage[indexOfContainsInList(lastPage, "bank"):], "Bank")][0].split("\n")
        return {
            "bank_name": bankInfo[indexOfContainsInList(bankInfo, "Bank Name ")].split(":")[-1].strip(),
            "account_number": bankInfo[indexOfContainsInList(bankInfo, "A/c N")].split(":")[-1].strip(),
            "ifs_code": bankInfo[indexOfContainsInList(bankInfo, "Branch & IFS")].split(":")[-1].strip(),
        }
