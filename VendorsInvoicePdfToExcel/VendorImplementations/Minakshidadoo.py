...
class Minakshidadoo:
    ...
    def getVendorInfo(self):
        ...
        return {
            ...
            "vendor_address": firstPageText[1],
            "vendor_mob": firstPageText[indexOfContainsInList(firstPageText, "Tel:")],
            "vendor_gst": firstPageText[2].split(":")[1],
            "vendor_email": "N/A"
        }
    ...
    def getInvoiceInfo(self):
        firstPageText = self.text_data[1].split("\n")
        return {
            "invoice_number": firstPageText[indexOfContainsInList(firstPageText, "Invoice No")].split(":")[1].split()[0],
            "invoice_date": firstPageText[indexOfContainsInList(firstPageText, "Invoice date:")].split(":")[1].split(" ")[1]
        }

    def getReceiverInfo(self):
        firstPageText = self.text_data[1].split("\n")

        indexOfReceive = indexOfContainsInList(firstPageText, "Ship to Party")
        indexOfBill = indexOfContainsInList(firstPageText, "Bill to Party")
        indexOfAmount = indexOfContainsInList(firstPageText, "Amount")

        sublist = firstPageText[indexOfBill: indexOfAmount]  # Extract the relevant section
        gst_index = indexOfContainsInList(sublist, "GSTIN")
        gst = sublist[gst_index].split(":")[-1].strip() if gst_index != -1 else None
        GST = re.sub(r"[^\w]", "", gst)

        return {
            "receiver_name": firstPageText[indexOfReceive + 1].split("Name:")[-1].strip(),
            "receiver_address": firstPageText[indexOfReceive + 2].split("Address:")[-1].strip(),
            "receiver_gst": GST
        }
    ...
    def getBillingInfo(self):
        ...
        return {
            "billto_name":substring_before_second_occurrence(firstPageText[indexOfReceiver + 1].split("Name :")[-1],"name"),
